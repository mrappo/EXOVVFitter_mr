...
#           cmd = "python g1_exo_doFit_class.py -b -c %s --mass %i --category %s --sample %s_lvjj --jetalgo %s --interpolate True > log/%s_M%i_%s_%s.log" %(options.channel,options.ntuple,m,options.category,sample,options.jetalgo,sample,m,options.channel,options.category)
                  cmd_tmp = "python MATTEO_g1_exo_doFit_class_new2.py -b -c %s --ntuple %s --mass %i --category %s --sample %s --jetalgo %s --luminosity %f --vbf True %s > " %(options.channel,options.ntuple,m,options.category,sample,options.jetalgo,lumi_float_value,pd_option)
...
#      outScript.write("\n"+'rm *.out');
                  ...
                  os.system("bsub -q cmscaf1nd -cwd "+currentDir+" "+currentDir+"/"+fn+".sh");
                    ...
                    cmd_tmp = "python MATTEO_g1_exo_doFit_class_new2.py -b -c %s --ntuple %s --mass %i --category %s --sample %s --jetalgo %s --luminosity %f --interpolate True --vbf True %s > " %(options.channel,options.ntuple,m,options.category,sample,options.jetalgo,lumi_float_value,pd_option)
                    cmd=cmd_tmp+log_file
                    print cmd
                    os.system(cmd)
            ...
            #### No VBF               
               ...
               if (options.interpolate==False and options.batchMode==True):
                  ...
                  outScript.write("\n"+'ls');
#           cmd = "python g1_exo_doFit_class.py -b -c %s --mass %i --category %s --sample %s_lvjj --jetalgo %s --interpolate True > log/%s_M%i_%s_%s.log" %(options.channel,options.ntuple,m,options.category,sample,options.jetalgo,sample,m,options.channel,options.category)
                  cmd_tmp = "python MATTEO_g1_exo_doFit_class_new2.py -b -c %s --ntuple %s --mass %i --category %s --sample %s --jetalgo %s --luminosity %f %s > " %(options.channel,options.ntuple,m,options.category,sample,options.jetalgo,lumi_float_value,pd_option)
                  cmd= cmd_tmp+log_file;
                  outScript.write("\n"+cmd);
#      outScript.write("\n"+'rm *.out');
                  outScript.close();
                  ...
                  os.system("bsub -q cmscaf1nd -cwd "+currentDir+" "+currentDir+"/"+fn+".sh");
                    ...
                    cmd_tmp = "python MATTEO_g1_exo_doFit_class_new2.py -b -c %s --ntuple %s --mass %i --category %s --sample %s --jetalgo %s --luminosity %f --interpolate True %s > " %(options.channel,options.ntuple,m,options.category,sample,options.jetalgo,lumi_float_value,pd_option)
                    cmd=cmd_tmp+log_file
                    print cmd
                    os.system(cmd)
               ...
               if (options.interpolate==False and options.batchMode==True):
                  ...
                  outScript.write("\n"+'ls');
#           cmd = "python g1_exo_doFit_class.py -b -c %s --mass %i --category %s --sample %s_lvjj --jetalgo %s --interpolate True > log/%s_M%i_%s_%s.log" %(options.channel,options.ntuple,m,options.category,sample,options.jetalgo,sample,m,options.channel,options.category)
                  cmd_tmp = "python MATTEO_g1_exo_doFit_class_new2.py -b -c %s --ntuple %s --mass %i --category %s --sample %s --jetalgo %s --luminosity %f > " %(options.channel,options.ntuple,m,options.category,sample,options.jetalgo,lumi_float_value)
                  cmd=cmd_tmp+log_file;
                  outScript.write("\n"+cmd);
#      outScript.write("\n"+'rm *.out');
                  outScript.close();
                  ...
                  os.system("bsub -q cmscaf1nd -cwd "+currentDir+" "+currentDir+"/"+fn+".sh");
                    ...
                    cmd_tmp = "python MATTEO_g1_exo_doFit_class_new2.py -b -c %s --ntuple %s --mass %i --category %s --sample %s --jetalgo %s --luminosity %f --interpolate True > " %(options.channel,options.ntuple,m,options.category,sample,options.jetalgo,lumi_float_value)
                    cmd=cmd_tmp+log_file
                    print cmd
                    os.system(cmd)
